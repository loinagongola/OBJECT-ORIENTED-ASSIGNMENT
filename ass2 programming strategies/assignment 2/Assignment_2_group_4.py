...
while loop_back:
    ...
    user_time_period = str(input())
    if user_time_period in ["1", "2", "3", "4"]:
        user_time_period = int(user_time_period)
        ...
        # User time period selected which days to pick
        for day in time:
            print(f"for {day}")
            while True:
                user_product_number = str(input("Enter product number 1-5, or enter 0 to stop:\n "))
                if user_product_number in product_prices:
                    # Prevents user from picking invalid input
                    while True:
                        user_quantity = str(input("Enter quantity sold:\n "))
                        if user_quantity[0] not in integers:
